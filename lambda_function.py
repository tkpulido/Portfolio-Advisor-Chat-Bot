...
def validate_data(age, investment_amount, risk_level, intent_request):
    ...
    # Validate that the user's age is a value greter than zero and less than 65
    if age is not None:
        age = parse_int(
            age
        )         
        if age < 1:
            return build_validation_result(
                False,
                "age",
                "You need to enter a value greater than zero to use this service, "
        ...
        if age > 64:
            return build_validation_result(
                False,
                "age",
                "You need to be under the age of 65 to use this service, "
                "please provide a different age.",
            )
    ...
    if risk_level is not None:
        if risk_level not in ["None", "Low", "Medium", "High"]:
            return build_validation_result(
                False,
                "riskLevel",
                "Please enter an acceptable risk level of None, Low, Medium, or High",
            )

    # A True results is returned if age and amount are valid
...
def recommend_portfolio(intent_request):
    ...
    # Explicitly parse risk level as string
    ...
    if validation_result == build_validation_result(True, None, None):
        if risk_level == "None":
            return build_validation_result(
                True,
                "riskLevel",
